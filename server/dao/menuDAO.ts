...
export default class MenuDbClient{
    ...
    static async injectDB(connection : MongoClient){
        ...
        try{
            ...
            if( !WineItems )
                WineItems = await connection.db(config.dataBases.menu).collection('wine_items');
